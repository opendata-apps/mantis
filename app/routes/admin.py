...
_ALLDATA_REFRESH_INTERVAL = timedelta(minutes=1)
_alldata_refresh_lock = Lock()
_alldata_last_refreshed: datetime | None = None


def _maybe_refresh_alldata_view(*, force: bool = False) -> None:
    """Refresh the alldata materialized view if stale (>1 min) or forced.

    The view is a single global object and REFRESH takes an ACCESS EXCLUSIVE
    lock, so the throttle is process-wide. Keeping it per-session would give
    every reviewer their own window and multiply the refreshes.
    """
    global _alldata_last_refreshed

    with _alldata_refresh_lock:
        now = datetime.now()
        if (
            not force
            and _alldata_last_refreshed is not None
            and now - _alldata_last_refreshed <= _ALLDATA_REFRESH_INTERVAL
        ):
            return
        ad.refresh_materialized_view(db)
        _alldata_last_refreshed = now
...
def _alldata_column_routing() -> dict[str, tuple[Any, str]]:
    ...
    for model in _ALLDATA_SOURCE_MODELS:
        for column in model.__table__.columns:
            name = column.name
            if name in _ALLDATA_IGNORED_COLUMNS:
                continue
