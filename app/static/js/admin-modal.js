...
function initiateMap(data) {
  requestAnimationFrame(function () {
    ...
    L.control
      .layers({
        Karte: osmLayer,
        Satellit: L.layerGroup([esriImagery, esriLabels]),
      })
      .addTo(map);
...
function toggleMarkerPlacement() {
  ...
  if (!els.btn.classList.contains("bg-yellow-500")) {
    ...
    map.once("click", function () {
      els.confirmBtns.classList.remove("hidden");
    });
...
function handleCopyClick(button) {
  ...
  navigator.clipboard
    ...
    .then(function () {
      ...
      var checkPath = document.createElementNS(
      ...
      window.setTimeout(function () {
        var copyPath = document.createElementNS(
          "http://www.w3.org/2000/svg",
          "path",
        );
...
document.body.addEventListener("htmx:afterSettle", function (event) {
  if (
    ...
    event.detail.target?.id !== "modal-body"
  )
    return;
