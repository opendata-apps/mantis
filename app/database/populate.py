...
def populate_feedback_types(session: Session):
    ...
    for id, name in INITIAL_FEEDBACK_TYPE_DATA:
        try:
            ...
            if not existing:
                session.execute(
                    text("INSERT INTO feedback_types (id, name) VALUES (:id, :name)"),
                    {"id": id, "name": name},
                )
                ...
                current_app.logger.debug(
                    f"Feedback type record with id {id} or name '{name}' already exists."
                )
        except Exception as e:
    ...
    if count > 0:
        session.commit()
    current_app.logger.info(
        f"Feedback_types table population complete. Inserted {count} new records."
    )
