...
class TestReportPrefill:
    ...
    def test_prefill_with_full_name(self, client, session, test_user_data):
        ...
        try:
            ...
            assert 'value="Müller"' in response_text, "Last name should be prefilled"
            ...
            # Check for the prefill notification message (updated text)
            assert 'Ihre Daten wurden basierend auf Ihrem Benutzer-Account vorausgefüllt' in response_text, "Prefill notification should be shown"
            
            # Check that fields are readonly
            assert 'readonly' in response_text, "Prefilled fields should be readonly"
    ...
    def test_no_prefill_for_invalid_user_id(self, client):
        ...
        # Should not show prefill notification
        assert 'Ihre Daten wurden basierend auf Ihrem Benutzer-Account vorausgefüllt' not in response_text, \
               "Prefill notification should not be shown for invalid user ID"
    ...
    def test_form_without_user_id(self, client):
        ...
        # Should not show prefill notification
        assert 'Ihre Daten wurden basierend auf Ihrem Benutzer-Account vorausgefüllt' not in response_text, \
               "Prefill notification should not be shown without user ID"
    ...
    def test_prefilled_fields_are_readonly(self, client, session, test_user_data):
        ...
        try:
            ...
            # Check that fields have the readonly styling
            assert 'bg-gray-100' in response_text, "Prefilled fields should have readonly styling"
            assert 'cursor-not-allowed' in response_text, "Prefilled fields should show not-allowed cursor"
